...
/*
 * lombok : setter, getter, toString, 생성자 등을 자동 생성해주는 유틸리티.
 * 
 * lombok 사용
 * 		- lombok 설치 : https://projectlombok.org > download => lombok.jar 파일 다운받기
 * 			cmd 창 열기 => lombok.jar 파일의 폴더로 이동
 * 			java - jar lombok.jar 실행
 * 			eclipse.exe 선택 후 install 하기
 * 		- lombok 관련 jar file 붙이기
 * 			pom.xml에 설정
 * 
 * lombok에서 사용하는 어노테이션 설명
 * 		@Setter : 자동으로 setter(setXxxx) 소스 생성
 * 		@Getter : 자동으로 getter(getXxxx) 소스 생성
