...
*/		
	 ...
	 * kakao 결제 : ajax으로 요청됨. =>
	 			...
	 			merchant_uid : json.merchant_uid,	//주문번호	: 주문멸로 유일한값이 필요.	<userid-session id값>
	 			name : json.name,	//주문상품명. 사과외 3건
	 			amount : json.amount,	//전체주문금액
	 			//주문자에 대한 정보 
	 			buyer_email : "jjeongjuri@naver.com",	//주문자 이메일.테스트.
	...
	public Map<String,Object> kakao(HttpSession session) {
		...
		map.put("buyer_email", email);	//현재사용안함. 복호화 필요
	...
	public ModelAndView checkend(HttpSession session) {
		...
		Cart cart = (Cart)session.getAttribute("CART"); //카트정보
		User loginUser = (User)session.getAttribute("loginUser"); //로그인정보
		//sale : 주문정보, 아이디정보, 상품목록, 상품정보
		Sale sale = service.checkend(loginUser,cart); //카트,로그인 서비스로전달
